package com.carworkz.dearo.customviews.toggle.widget;
...
public class LabeledSwitch extends ToggleableView
{
    ...
    private int colorCircleOn;
    private int colorCircleOff;
    ...
    private void initProperties(AttributeSet attrs)
    {
        ...
        for (int i = 0; i < N; ++i)
        {
            ...
            else if (attr == R.styleable.Toggle_colorCircleOn)
            {
                int accentColor;
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M)
                {
                ...
                colorCircleOn = tarr.getColor(R.styleable.Toggle_colorCircleOn, accentColor);
            }
            else if (attr == R.styleable.Toggle_colorCircleOff)
            {
                int accentColor;
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M)
                {
                ...
                colorCircleOff = tarr.getColor(R.styleable.Toggle_colorCircleOff, accentColor);
            }
        }
    }
...
//      Drawing Switch background here
        {
            ...
            else
            {
                if(this.isOn)
                {
                    paint.setColor(colorCircleOn);
                }
                else
                {
                    paint.setColor(colorCircleOff);
                }
           ...
           /* if (!isEnabled())
            {
             ...
             if(this.isOn)
            {
                onColor = colorCircleOn;
            }
            else
            {
                onColor = colorCircleOff;
            }
            paint.setColor(onColor);
            canvas.drawCircle(thumbBounds.centerX(), thumbBounds.centerY(), thumbRadii, paint);
        }
